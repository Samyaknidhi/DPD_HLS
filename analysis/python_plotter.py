...
# Load PA output with DPD (new addition)
...
# Calculate NMSE between DPD input and PA output (new)
n_show = min(len(pa_with_dpd), len(dpd_i_up))
nmse_pa_dpd = 10 * np.log10(np.mean((dpd_i_up[:n_show] - pa_with_dpd[:n_show])**2) / np.mean(dpd_i_up[:n_show]**2))
...
ax5.hist(pa_with_dpd, bins=100, alpha=0.5, color='red', label='PA w/DPD')  # New: PA with DPD histogram
...
n_show = min(1000, len(ddc_i), len(pa_with_dpd))  # Show up to 1000 points for clarity
ax6.plot(dpd_i_up[:n_show], label='DPD Input I')
ax6.plot(pa_with_dpd[:n_show], label='PA Output w/DPD', color='red')  # New: PA with DPD
...
norm_pa_dpd = pa_with_dpd[:n_show] / np.max(np.abs(pa_with_dpd[:n_show]))  # New: Normalized PA output
ax7.plot(norm_input_i, label='Normalized Input')
ax7.plot(norm_pa_dpd, label='Normalized PA w/DPD', color='red')  # New: Plot normalized PA output
...
ax8.scatter(norm_input_i, norm_pa_dpd, s=1, alpha=0.5, color='red')  # New: Input vs PA correlation
ax8.set_title('Input vs PA Output Correlation')
...
# 9. NEW: PA Output with DPD - Magnitude Analysis
ax9 = fig.add_subplot(grid[4, 0])
ax9.plot(pa_with_dpd[:n_show], color='red')
...
ax9.set_title('PA Output with DPD Analysis')
ax9.set_xlabel('Sample')
ax9.set_ylabel('Amplitude')
ax9.legend()
ax9.grid(True)

# 10. NEW: DPD Input vs PA Output (Direct Comparison)
ax10 = fig.add_subplot(grid[4, 1])
# Calculate gain/scale factor
scale_factor = np.mean(np.abs(pa_with_dpd[:n_show])) / np.mean(np.abs(dpd_i_up[:n_show]))
scaled_input = dpd_i_up[:n_show] * scale_factor
ax10.plot(scaled_input, label=f'Scaled Input (×{scale_factor:.2f})')
ax10.plot(pa_with_dpd[:n_show], color='red', label='PA Output w/DPD', alpha=0.7)
